...
def get_or_create_user():
    username = input("Введите ваш ник: ").strip()
    ...
    if user:
        user_id = user[0]
        print(f"Привет, {username}! Загружаем твой прогресс...")
    else:
...
while running:
    ...
    if keys[pygame.K_UP] and dy == 0:
        ...
        dx, dy = size, 0
    elif keys[pygame.K_p]:  # -------------------- Пауза и сохранение --------------------
        paused = True
        save_game(user_id, score, level, snake, dx, dy, food, food_weight)
        while paused:
            ...
            if keys2[pygame.K_r]:  # Нажми R, чтобы продолжить
                paused = False
            show_text("PAUSED! Нажмите R для продолжения", RED, (W//2 - 180, H//2))
            pygame.display.flip()
            clock.tick(5)
